...
const ColorPicker = ({ open, onClose, mode, customCursorEnabled, setCustomCursorEnabled }) => {
  ...
  useEffect(() => {
    if (open) {
      ...
      // Only populate scrollbar and cursor values on desktop
      if (!isMobile) {
  ...
  const handleReset = () => {
    ...
    // Only include scrollbar and cursor colors on desktop
    if (!isMobile) {
      defaultColors.scrollbar = {
        track: {
          dark: '#0a192f',
          light: '#f5f7fa',
        },
    ...
    Object.keys(defaultColors).forEach(category => {
      // Skip scrollbar and cursor categories on mobile
      if (isMobile && (category === 'scrollbar' || category === 'cursor')) {
        return;
      }
  ...
  const renderModeColors = (mode) => {
    ...
    return (
      <Grid container spacing={2}>
        ...
        {/* Scrollbar - Only show on desktop */}
        {!isMobile && (
        <Grid item xs={12}>
        ...
        {/* Cursor - Only show if custom cursor is enabled AND not on mobile */}
        {customCursorEnabled && !isMobile && (
          <Grid item xs={12}>
