...
const Header = ({ toggleTheme, mode, customCursorEnabled, setCustomCursorEnabled }) => {
  ...
  const drawer = (
    <List>
      ...
      <ListItem button onClick={handleColorPickerOpen}>
  ...
  return (
    ...
    <AppBar position="fixed" color="transparent" elevation={0} sx={{ backdropFilter: 'blur(10px)' }}>
      <Toolbar>
        ...
        <Box sx={{ display: { xs: 'none', md: 'flex' }, alignItems: 'center' }}>
          ...
          <IconButton sx={{ ml: 2 }} onClick={toggleTheme} color="inherit">
            ...
            <IconButton sx={{ ml: 1 }} onClick={handleColorPickerOpen} color="inherit">
              <PaletteIcon />
            </IconButton>
        </Box>
        <IconButton
      ...
      <ColorPicker open={colorPickerOpen} onClose={handleColorPickerClose} mode={mode} customCursorEnabled={customCursorEnabled} setCustomCursorEnabled={setCustomCursorEnabled} />
    </>
  );
};
